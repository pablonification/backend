...
const db = {
  ...
  async getStudent(id) {
    const { data, error } = await supabase
      .from('students')
      .select('id, email, full_name, nim, cohort, faculty, avatar_url')
      .eq('id', id)
      .single()
    return { data, error }
  },
  ...
  async createPushToken(tokenData) {
    const { data, error } = await supabase
      .from('push_tokens')
      .insert(tokenData)
      .select()
  ...
  async upsertPushToken(tokenData) {
    const { data, error } = await supabase
      .from('push_tokens')
      .upsert(tokenData, { onConflict: 'push_token' })
      .select()
      .single()
    return { data, error }
  },
  ...
  async getPushTokensByUser(userId, userType) {
    const { data, error } = await supabase
      .from('push_tokens')
      .select('*')
      .eq('user_id', userId)
